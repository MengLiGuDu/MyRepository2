...
<body>
	...
	<div class="line_div"><label>待确认收入</label></div>
	<div class="box">
	...
	<div class="modal" id="myAffirm">
		<div class="modal-dialog modal-content">
			<table width="300"height="200" style="margin-left:140px;">
				<tr>
					<td align="center"id="alert_font" colspan="2"style="font-family:'华文行楷';font-size:20px;">您确认要执行本操作吗？</td>
				</tr>
				<tr>
					<td align="center"><input onclick="exit()" type="button" class="btn btn-default" style="width: 100px;" value="取 消" ></td>
					<td align="center"><input onclick="affirm()" type="button" class="btn btn-info" style="width: 100px;" value="确 认" /></td>
				</tr>
			</table>
			<input id="income_id" style="display: none;">
		</div>
	</div>
</body>
...
//确认
function affirm(){
	$.post("<%=path%>/income_wait_affirm_state.action",{income_id:$("#income_id").val()},function(list){
		exit();
		refresh();
	},"json")
}
...
function wiatTable(value){
	...
	$('#fm-table').bootstrapTable({
		...
		url: "<%=path%>/income_wait_confirmed.action?searchText2="+value+"",
		method:"post",
		dataType: "json",
		toolbar:'#toolbar',
		striped:true,//隔行变色
        ...
        columns: [{
            ...
            align: 'center',
            formatter:function(value,row,index){
               	var e = '<button  class="btn btn-info"  onclick="aff(\''+value+'\')"><span class="glyphicon glyphicon-ok"></span> 确认收入</button> ';
                var d = '<button  class="btn btn-danger"  onclick="look(\''+value+'\')" ><span class="glyphicon glyphicon-list"></span> 查看明细</button> ';
                return e+d;
            }
        }],
